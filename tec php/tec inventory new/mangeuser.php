        ...
        <div class="container-fluid">
          ...
          <div class="collapse navbar-collapse justify-content-end" id="navigation">
            <ul class="navbar-nav">
              <?php navigation($conn); ?>
              <li class="nav_search">
                <form action="mangeuser.php" method="post">
                  <div class="input-group no-border">
                    <input list="brow" value="" class="form-control" placeholder="Search..." id="search"><!---add name get text in search -->
                    <datalist id="brow">
                    ...
                    <div class="input-group-append">
                      <div class="input-group-text">
                        <i type="button" name="searchbtn" class="nc-icon nc-zoom-split"></i>
                      </div>
                    </div>
                  </div>
            ...
            <div class="card card-plain">
              <div class="card-header">
                ...
                <div>
                    ...
                    <select class="cat_select" id="position" name="position">
                      <option class="cat_select_option" value="0">All Position</option>
                     <?php 
                        $query = "SELECT distinct(job_position) FROM staff";
                        $result = mysqli_query($conn,$query);
                        if ($result) {
                          if (mysqli_num_rows($result)>0) {
                            while ($detail = mysqli_fetch_assoc($result)) {
                              echo "<option class=\"month_select_option\" value=\"".$detail['job_position']."\">".$detail['job_position']."</option>";
                            }
                          }
                        }
  ...
  <script>
    $(document).ready(function() {
      ...
      $("#search").keyup(function(){
        ...
        var jposition = $("#position").val();
        var jdepartment = $("#department").val();
        $.ajax({
          type:"post",
          url:"mangeuser_ajax.php",
          data:{position:jposition,department:jdepartment,search:jsearch},
          success:function(data){
            table=$.parseJSON(data);
            $('#table').html(table);
          }
